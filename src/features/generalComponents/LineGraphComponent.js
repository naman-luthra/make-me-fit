            ...
            <div className="md:flex md:gap-8 md:mt-4">
                <svg height={height+60} width={width+80} viewBox={`0 0 ${width+40} ${height+60}`} className={`grow -translate-x-3 md:translate-x-0 ${graphs[0].prediction ? 'w-full h-full md:w-2/3' : 'w-full h-full'}`}>
                    ...
                    }
                    {
                        ...
                        <text
                            textAnchor="end"
                            alignmentBaseline={target.type==='gt' ? "hanging" : "baseline"}
                            x={55+width}
                            y={target.type==='gt' ? 25 : height+15}
                            fontSize="12"
                            fontWeight={600}
                        >
                            {target.label}
                            ...
                            <>
                                    ...
                                    graph.dataPolygonPoints.map((p,i)=>(
                                        <>
                                            <circle
                                                ...
                                                onMouseEnter={()=>{
                                                    document.querySelectorAll(`.${graphID}-text-${i}`).forEach(e=>{
                                ...
                                }
                                {
                                    graph.dataPolygonPoints.map((p,i)=>(
                                        <>
                                            <rect 
                                                x={p[0]+5} y={p[1]-5}  width={graph.hoverLabel(graph.data[i]).length*7+10} height="20" rx="5" className={`${graphID}-text-${i} opacity-0 hidden fill-gray-600 cursor-default transition-all duration-300`}
                                            />
                                            <text x={p[0]+10} y={p[1]+10} fill="white" fontSize={12} fontWeight={700} className={`${graphID}-text-${i} opacity-0 hidden cursor-default transition-all duration-300`}>
